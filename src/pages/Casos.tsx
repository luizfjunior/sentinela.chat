...
export default function Casos() {
  ...
  const CaseCard = ({ caso, index }: { caso: Case; index: number }) => (
    <Draggable draggableId={caso.id} index={index}>
      {(provided, snapshot) => (
        ...
        >
          <Card 
            className={`bg-background border-border hover:border-primary/50 transition-all cursor-grab active:cursor-grabbing ${
