...
const Sidebar = ({
  ...
  return <>
    {/* Toggle Button */}
    <Button variant="ghost" size="sm" onClick={onToggle} className="fixed top-3 left-4 z-50 text-white p-2 flex items-center justify-center bg-transparent rounded-none text-lg font-bold">
    ...
    <div className={cn("fixed left-0 top-0 h-full bg-zinc-900 border-r border-zinc-800 transition-transform duration-300 z-40", isOpen ? "translate-x-0" : "-translate-x-full", "w-64")}>
      ...
      <div className="flex-1 overflow-y-auto">
        {conversations.map(conversation => <div key={conversation.id} className={cn("group flex items-center justify-between px-4 py-2 mx-2 rounded-lg cursor-pointer hover:bg-zinc-800 transition-colors", currentConversationId === conversation.id && "bg-zinc-800")} onClick={() => editingId !== conversation.id && onSelectConversation(conversation.id)}>
            {editingId === conversation.id ? <input type="text" value={editingTitle} onChange={e => setEditingTitle(e.target.value)} onBlur={handleSaveEdit} onKeyDown={handleKeyDown} className="bg-zinc-700 text-white text-sm rounded px-2 py-1 flex-1 focus:outline-none focus:ring-2 focus:ring-blue-500" autoFocus onClick={e => e.stopPropagation()} /> : <span className="text-white text-sm truncate flex-1">
                {conversation.title}
              </span>}
